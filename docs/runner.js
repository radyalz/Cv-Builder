...
/* -------------------------------------------------------------------------
   Skull artwork
   Traced from the concept art. The skull is an SVG image built here so its
   colours can follow the accent, which an image cannot read from CSS. The
   motion lives inside the SVG, so every tile moves together from one image.
   ---------------------------------------------------------------------- */
...
function skullArt(accent) {
  const { cranium, jaw } = skullPaths();

  const boneLight = mixHex(accent, "#f2ebe3", 0.78);
  const boneMid = mixHex(accent, "#7d736d", 0.62);
  const boneDark = mixHex(accent, "#110c0b", 0.72);
  const rim = mixHex(accent, "#ffffff", 0.25);
  const hot = mixHex(accent, "#ffffff", 0.55);

  const motion = REDUCED_MOTION.matches
    ? ""
    : "<style>" +
      ".j{animation:j 7s ease-in-out infinite}" +
      ".e{animation:e 6s ease-in-out infinite}" +
      "@keyframes j{0%,30%,100%{transform:translateY(0)}55%,72%{transform:translateY(2.4px)}}" +
      "@keyframes e{0%,100%{opacity:.5}50%{opacity:1}}" +
      "</style>";

  const layer = (id) =>
    `<use href="#${id}" fill="none" stroke="${accent}" stroke-width="2.2" ` +
    `stroke-opacity=".2" stroke-linejoin="round"/>` +
    `<use href="#${id}" fill="url(#b)" stroke="${rim}" stroke-width=".35" stroke-opacity=".75"/>`;
  ...
  const svg =
    ...
    motion +
    '<g class="e">' +
    '<circle cx="38" cy="37" r="7.5" fill="url(#g)"/>' +
    '<circle cx="63" cy="37" r="7.5" fill="url(#g)"/>' +
    "</g>" +
    layer("c") +
    `<g class="j">${layer("k")}</g>` +
    "</svg>";
...
function paintSkulls(accent) {
  ...
  skullAccent = accent;
  document.documentElement.style.setProperty("--skull-art", skullArt(accent));
}
